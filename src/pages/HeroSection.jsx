import { useState, useEffect  } from "react"; 
import HeroLogo from "../assets/hero.webp";
import{ cards } from "../hooks/usemetrics.js";
import MetricsCard from "../components/cards/MetricCard";
import DualAxisChart from "./DualAxisChart";    
import { nominalGdpData } from "../hooks/transform_data.js";

export default function HeroSection() {

    const [gdpValue, setGdpValue] = useState(null);
    const lastHdiData = cards[0].chartData.findLast(item => item.rank !== null);
    const hdiValue = lastHdiData ? lastHdiData.rank : null;

useEffect(() => {
    const fetchGdpData = async () => {
        try {
            const  { formattedData } = await nominalGdpData();
            if (formattedData && formattedData.length > 0) {
                const lastRankEntry = formattedData[formattedData.length - 1];
                setGdpValue(lastRankEntry.rank);
            }
        } catch (error) {
            console.error("Error fetching GDP data:", error);
        }
    }
    fetchGdpData();
}, [])



    const NIGERIA = "Nigeria";
    const ArrayTitle = NIGERIA.split("")
    const COLORS = ['#22c55e', '#F0EDE6']

    const mapArrayTitle = ArrayTitle.map((letter, index) => (
        <span key={index}
        style={{
            color: COLORS[index & letter.length]
        }}>
            {letter}
        </span>
    ))
    return(
        <div>
            {/* HERO SECTION */}
            <div className="relative min-h-[350px]">
                
                {/* ✅ Optimized hero image */}
                <img
                src={HeroLogo}
                alt="Hero"
                fetchpriority="high"
                loading="eager"
                className="absolute inset-0 w-full h-full object-cover"
                />

                {/* H1 */}
                <div className="absolute inset-0 bg-black/60 p-6">
                    <h1 className="font-['Merriweather'] font-extrabold text-4xl md:text-6xl mb-5 leading-snug">
                        {mapArrayTitle} is the {gdpValue !== null ? gdpValue : "null"}{" "}
                        <strong className="text-[#F59E0B]">largest economy</strong> in the{" "}
                        <span className="md:[text-shadow:_2px_2px_8px_rgba(255,247,247,0.8)]">
                        World
                        </span>
                    </h1>

                    <span className="text-[#F0EDE6] text-shadow-lg text-shadow-black md:text-2xl">
                        Yet it ranks{" "}
                        <strong className="text-[#D9534F]">
                        {hdiValue !== null ? hdiValue : "null"}
                        </strong>{" "}
                        in human development&mdash; the gap reveals how growth alone rarely lifts
                        people out of poverty.
                    </span>
                </div>
            </div>

            {/* METRICS */}
            <div>
                <MetricsCard mockData={cards} />
            </div>

            {/* CHART */}
            <div className="flex items-center justify-center w-full">
                <DualAxisChart />
            </div>
        </div>
        
    )
}